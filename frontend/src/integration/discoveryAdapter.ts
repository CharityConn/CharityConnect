import { ITokenCollection, TokenType } from '@tokenscript/engine-js/dist/lib.esm/tokens/ITokenCollection';
...
export class DiscoveryAdapter implements ITokenDiscoveryAdapter {
  ...
  async getCachedTokens(initialTokenDetails: ITokenCollection, ownerAddress: string): Promise<ITokenCollection | false> {
    if (!this.enableStorage) return false;

    try {
      const token = await dbProvider.tokens2
        .where({
      ...
      if (token && Date.now() < token.dt + TOKEN_CACHE_TTL * 1000) return token.data;
    } catch (e) {
      console.warn(e);
    }
  ...
  async fetchTokens(token: ITokenCollection, ownerAddress: string) {
    if (!CHAIN_MAP[token.chainId]) throw new Error('Chain ID ' + token.chainId + ' is not supported for token discovery');
    ...
    if (collectionData?.image) token.image = collectionData.image;
  ...
  private async getCachedMeta(token: ITokenCollection) {
    ...
    try {
      const tokenMeta = await dbProvider.tokenMeta2
        .where({
          chainId: token.chainId,
          collectionId: token.contractAddress.toLowerCase(),
          tokenType: token.tokenType,
      ...
      if (tokenMeta && Date.now() < tokenMeta.dt + COLLECTION_CACHE_TTL * 1000) return tokenMeta.data;
    } catch (e) {
      console.warn(e);
    }
  ...
  private async fetchTokenMetadataRpc(token: ITokenCollection) {
    ...
    if (contractUri) {
      const contractMeta = await (
        await fetch(contractUri, {
          headers: {
            Accept: 'text/plain',
          },
        })
      ).json();
      if (contractMeta.name || contractMeta.title) name = contractMeta.name ?? contractMeta.title;
      if (contractMeta.description) description = contractMeta.description;
      if (contractMeta.image) image = contractMeta.image;
    }
  ...
  private async getTokenIdsLogs(contract, owner) {
    const sentLogs = await contract.queryFilter(contract.filters.Transfer(owner, null));
    const receivedLogs = await contract.queryFilter(contract.filters.Transfer(null, owner));
