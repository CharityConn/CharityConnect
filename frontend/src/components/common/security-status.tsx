...
import { ISecurityInfo, SecurityStatus as TSSecurityStatus } from '@tokenscript/engine-js/dist/lib.esm/security/SecurityInfo';
...
export class SecurityStatus {
  ...
  private getDetailedSecurityInfo() {
    const authMethods = this.securityInfo.originStatuses.reduce((previous, originStatus) => {
      ...
      if (previous.indexOf(originStatus.type) === -1) previous.push(originStatus.type);
      return previous;
    }, []);
  ...
  render() {
    return this.securityInfo ? (
      <Host>
        <div
          class={'security-status' + (this.size === 'small' ? ' small' : '')}
