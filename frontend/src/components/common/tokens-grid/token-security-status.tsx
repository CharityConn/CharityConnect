...
import { SecurityStatus as TSSecurityStatus } from '@tokenscript/engine-js/dist/lib.esm/security/SecurityInfo';
import { TokenScript } from '@tokenscript/engine-js/dist/lib.esm/TokenScript';
import { IOriginSecurityInfo } from '@tokenscript/engine-js/dist/lib.esm/tokenScript/Origin';
...
export class TokenSecurityStatus {
  ...
  private getDetailedSecurityInfo() {
    return (
      (this.securityInfo.trustedKey ? 'Issued by: ' + this.securityInfo.trustedKey.issuerName + '\n\n' + 'Signer key: ' + this.securityInfo.trustedKey.value : '') +
  ...
  render() {
    return this.securityInfo ? (
      <Host>
        <div
          class="token-security-status"
          style={{ background: this.statusColor }}
