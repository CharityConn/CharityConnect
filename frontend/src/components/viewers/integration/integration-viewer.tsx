...
export class IntegrationViewer {
  ...
  private verifyRequest() {
    if (!this.urlRequest.has('callbackUri')) {
      ...
      if (!this.messageTarget) throw new Error('Cannot return message to requester, no callbackUrl supplied or window parent & opener is null.');
    }
  }
  ...
  private async openTokenScript(source: TokenScriptSource, tsId?: string) {
    ...
    try {
      ...
      if (!this.card) throw new Error('Card ' + this.urlRequest.get('card') + ' could not be found.');
