import { IAttestationData } from '@tokenscript/engine-js/dist/lib.esm/attestation/IAttestationStorageAdapter';
  ...
  async cloneDatabase(sourceName, destinationName) {
    ...
    return origDb.open().then(() => {
      ...
      return origDb.tables
        .reduce(
          (prev, table) => prev.then(() => table.toArray()).then(rows => destDb.table(table.name).bulkAdd(rows)),
